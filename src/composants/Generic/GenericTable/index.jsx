...
const GenericTable = ({name, displayedColumns, setDisplayedColumns}) => {

  // States et méthodes partagés par mon provider
  const { setErrors, fetchData, authToken, authUser, authPermissions, cache, updateCache } = useApi();
  ...
  const handleCheckboxChange = (entry) => {
    ...
    setSelectedEntries((prevEntries) => {
      ...
      const updatedEntries = prevEntries.includes(entry)
        ? prevEntries.filter((name) => name !== entry)
        : [...prevEntries, entry];
  ...
  const toggleSortOrder = (columnValue) => {
    ...
    const sortedData = cachedData.sort((a, b) => {
        ...
        if (name === 'users' && (columnValue === 'hireDate' || columnValue === 'endDate')) {
            ...
            aValue = a.hasOwnProperty(columnValue) ? a[columnValue] : '';
            bValue = b.hasOwnProperty(columnValue) ? b[columnValue] : '';
        }
    ...
    name === 'users' ?
        updateCache('http://localhost:8000/api/users', { ...cache['http://localhost:8000/api/users'], 'hydra:member': sortedData })
        : updateCache('http://localhost:8000/api/ingredients', { ...cache['http://localhost:8000/api/ingredients'], 'hydra:member': sortedData });
                ...
                */}
                {entries.map((entry) => (
                  <tr key={entry.id} >
                    ...
                    <td className={`text-center ${name === 'ingredients' ? getTextColorClass(entry.percentQuantity) : ''}`}>{name === 'users' ? entry.realName : name === 'ingredients' ? entry.title : ''}</td>
