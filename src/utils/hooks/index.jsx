...
export const useApiRequest = (cache, updateCache) => {
  ...
  // Méthode permettant de requêter l'API, de modifier le cache et de renvoyer la réponse au composant y faisant appel
...
export const useAssociativeEntityUpdater = (fetchData, authToken, updateUserAuth) => {
  ...
  const isArraySuperset = (initialsecondEntityIds, secondEntityIds) => {
    for (let elem of secondEntityIds) {
      if (!initialsecondEntityIds.includes(elem)) {
        return false;
      }
    }
    return true;
...
export const useAuth = ( setIsLoading, cache, fetchData) => {
  ...
  const updateUserAuth = async (authToken, authRoles = [], authUser = null, isRetrievedFromLS = false) => {
    console.log('2');
    ...
    */
    ...
    if (authRoles.length === 0 && !authUser) {
  ...
  */
    ...
    const getPermissionsForRoles = () => {
      ...
      if (rolesCollection ?? false) {
        ...
        authRoles.forEach(authRole => {
          ...
          const role = rolesCollection['hydra:member'].find(role => role.name === authRole);
  ...
  /* 
    React me conseille de mettre en dépendance 'cache' or le useeffect ne doit se 
    déclencher que si les rôles de l'utilisateur sont modifiés. Je retire donc l'avertissement
  */
  ...
  */
  ...
  /* 
    React me conseille de mettre en dépendance updateUserAuth or le useeffect
    ne doit se déclencher qu'au montage initial du composant. Je retire donc l'avertissement
  */
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return { authToken, authRoles, authUser, authPermissions, updateUserAuth };
}

// Hook permettant d'accéder aux informations partagées par le context ApiContext
export const useApi = () => {
  const { isLoading, setIsLoading, cache, updateCache, errors, setErrors, fetchData, authToken, authRoles, authUser, authPermissions, updateUserAuth, isArraySuperset, updateAssociativeEntity  } = useContext(ApiContext);
  return { isLoading, setIsLoading, cache, updateCache, errors, setErrors, fetchData, authToken, authRoles, authUser, authPermissions, updateUserAuth, isArraySuperset, updateAssociativeEntity };
};
